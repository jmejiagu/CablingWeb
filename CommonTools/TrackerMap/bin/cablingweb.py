...
############THESE INSTRUCTIONS ARE FOR GETTING THE INFO OF THE DICTIONARY FOR THE CABLING FILE#############################3
    ...
    if options.aldet1:
        ...
        archi1.close()
        archi2.close()
        os.system(('print_TrackerMap %s "TrackerMap" %s True 2400 False False 999 -999')%(options.aldet1,options.aldet3))
...
###Tracker map of some modules
    if options.lismod:
        ...
        os.system(('print_TrackerMap %s  "TrackerMap" %s True 2400 False False 999 -999')%(options.det4,options.det5))
        ...
        if options.lisfetrc1=="FedId":
            ...
            for x in lisf1:
                if x not in lisf1n:
                    ...
                    txtc1.write(x+"\n")
            txtc1.close()
            os.system(('print_TrackerMap %s  "TrackerMap" %s True 2400 False False 999 -999')%(options.fe3,options.fe4))
...
def HVInfoDictF(filenameC,filename, options):
   ...
   #######A file with the modules associated to some values and a tracker map
    ...
    if options.vatrcm:
        ...
        for i in lihv2:
            variable+='%s  ' % (i)
        os.system('print_TrackerMap %s "Trackermap" %s True 2400 False False 999 -999'%(options.ps1,options.ps3))
...
####These options are to get information from one source to another ###########
    ...
    if options.alitkm:
        ...
        for i,j in zip (lials,color_list):
            variable+='%s=%s  ' % (i,j)
        os.system('print_TrackerMap %s "Tracker map" %s True 2400 False False 999 -999' % (options.al1,options.al3))
